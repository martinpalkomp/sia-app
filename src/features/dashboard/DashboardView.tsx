...
export const DashboardView: React.FC<{
  ...
  stats: any;
  insights: any[];
  dailyBrief: string | AIInsight | null;
  dataMaturity: any;
  dataDepth: any;
  ...
  insightTeaser: string | AIInsight | null;
  setInsightTeaser: (teaser: string | AIInsight | null) => void;
    ...
    <div className="space-y-8">
      ...
      <div id="db-morning-briefing" className="w-full">
        <div id="morning-brief-card" className="border border-zinc-800/60 bg-[#0B0F17] rounded-2xl relative shadow-sm group hover:border-indigo-500/30 transition-all duration-300 overflow-hidden w-full min-h-[15svh] p-6 flex flex-col justify-center">
          ...
          <div className="relative z-10 p-2">
            <p id="temporal-label" className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">
              ...
              <div id="brief-body" className="space-y-4">
                ...
                {dailyBrief.recommendation && (
                  <div className="mt-2 text-sm text-indigo-200">
                    <span className="font-bold uppercase tracking-widest text-[10px] text-indigo-400 block mb-1">Recommendation</span>
                    {dailyBrief.recommendation}
                  </div>
                )}
                {dailyBrief.evidence && dailyBrief.evidence.length > 0 && (
                  <div className="mt-2 text-xs font-mono text-zinc-400">
                    {dailyBrief.evidence.map((ev, i) => <div key={i}>• {ev}</div>)}
                  </div>
                )}
              </div>
            )
      ...
      <section className="space-y-6 pt-6 border-t border-zinc-800">
        ...
        <button
          ...
          className="w-full text-left border border-cyan-500/20 bg-[#111827] rounded-2xl relative shadow-[0_0_15px_-3px_rgba(6,182,212,0.05)] hover:shadow-[0_0_20px_-3px_rgba(6,182,212,0.15)] group hover:border-cyan-500/40 transition-all duration-300 overflow-hidden min-h-[15svh] p-8"
        >
          <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl group-hover:bg-cyan-500/20 transition-colors pointer-events-none group-hover:animate-[ping_3s_cubic-bezier(0,0,0.2,1)_infinite]" />
      ...
      {!isEnhanced ? (
        ...
        <>
          <button
            ...
            className="w-full mt-8 border border-indigo-500/20 bg-[#111827] rounded-2xl relative shadow-[0_0_15px_-3px_rgba(99,102,241,0.05)] hover:shadow-[0_0_20px_-3px_rgba(99,102,241,0.15)] group hover:border-indigo-500/40 transition-all duration-300 overflow-hidden min-h-[15svh] p-8 text-left"
            id="ai-analysis-card"
          >
            <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-500/10 rounded-full blur-3xl group-hover:bg-indigo-500/20 transition-colors pointer-events-none group-hover:animate-[ping_3s_cubic-bezier(0,0,0.2,1)_infinite]" />
            <div className="relative z-10 flex items-center justify-between">
              <div className="space-y-4">
                ...
                {isAiLoading ? (
                  ...
                  <div className="space-y-4 max-w-sm">
                    {(() => {
                      if (typeof insightTeaser !== 'string') {
                        return (
                          <div className="space-y-4">
                            <div>
                              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mb-1 items-center flex gap-2">
                                Pattern 
                                {insightTeaser.confidence && (
                                  <span className={`px-2 py-0.5 rounded text-[8px] tracking-wider ${insightTeaser.confidence === 'high' ? 'bg-indigo-500/20 text-indigo-400' : 'bg-zinc-800 text-zinc-400'}`}>
                                    {insightTeaser.confidence.toUpperCase()} CONFIDENCE
                                  </span>
                                )}
                              </p>
                            ...
                            {insightTeaser.evidence && insightTeaser.evidence.length > 0 && (
                              <div>
                                ...
                                <ul className="text-xs font-mono text-zinc-400 space-y-2">
                                  {insightTeaser.evidence.map((signal: string, idx: number) => (
                            ...
                            {insightTeaser.recommendation && (
                              <div className="mt-4 pt-4 border-t border-indigo-500/10">
                                <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest mb-1">Recommendation</p>
                                <p className="text-sm text-indigo-100">{insightTeaser.recommendation}</p>
                              </div>
                            )}
                          </div>
                        );
              ...
              <div className="space-y-4">
                <div className="border border-zinc-800/60 bg-[#0B0F17] rounded-2xl relative shadow-sm group hover:border-emerald-500/30 transition-all duration-300 overflow-hidden w-full p-8 space-y-4 min-h-[15svh] flex flex-col justify-center">
                  ...
                  <div className="relative z-10">
                    ...
                    <h3 className="font-black text-xs tracking-widest text-emerald-400 uppercase flex items-center gap-2 mb-4">
                      <Activity size={14} />
                  ...
                  {isDeepAnalysis ? (
                    ...
                    <p className="text-lg font-serif italic text-zinc-300 leading-relaxed">{deepAnalysisResult.summary}</p>
                  ) : (
                    <button
                      onClick={handleDeepAnalysis}
                      className="w-full py-4 border border-indigo-500/30 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                    >
                ...
                {deepAnalysisResult && (
                  <div className="border border-emerald-500/20 bg-[#0B0F17] rounded-2xl relative shadow-[0_0_15px_-3px_rgba(16,185,129,0.05)] hover:shadow-[0_0_20px_-3px_rgba(16,185,129,0.15)] group hover:border-emerald-500/40 transition-all duration-300 overflow-hidden w-full p-6 space-y-4">
                    ...
                    <p className="relative z-10 text-emerald-100 font-sans tracking-tight leading-relaxed">{deepAnalysisResult.recommendation}</p>
                  </div>
                )}
